...
| TBS_Game_Godot（本仓库） | Godot 4.7 + GDScript | 中（核心层+基础战斗） | 迁移架构，重写代码，UI/流程待补齐 |

### 1.2 当前 Godot 版状态速览

- ✅ Godot 工程骨架（project.godot、autoload GameDatabase）
...
- ❌ 双战场（两块 3x4 分隔）—— 当前是单块 10x8
- ❌ 屏幕流程（主菜单→选关→成长→部署→战斗→结算）
- ❌ 成长系统 UI（属性/技能/装备子页）
- ❌ 字体管理、滚动列表组件（不做 i18n，仅中文版）
- ⚠️ 战斗界面可操作性需验证（历史记录显示"不能操作我方单位"问题曾出现）
...
## 8. 差距清单（Godot 版待补齐，按优先级）

### P0 战斗体验修复
1. **双战场恢复**：左右各 3x4 + 中间 gap，单位不可跨战场移动（对照原版 DualGrid 与跨战场攻击规则）
2. **战斗界面可操作性验证**：确认选中/移动/攻击/技能在真机（非 headless）可用；历史曾出现"不能操作我方单位"
3. **跨战场攻击距离规则**：攻击距离 = 双方到中线距离之和，忽略 Y 轴（原版 Stage 18 规则）
4. 移动范围应排除敌方占用格（已有逻辑，需验证）；补"无目标时取消"防卡死

### P1 战斗外流程
5. 主菜单（开始游戏/退出）→ 选关 → 部署 → 战斗 → 结算 屏幕链
6. 战斗外输入全部鼠标可点（原版曾要求不依赖特定按键）

### P2 成长与装备
7. 成长界面：属性加点 / 技能学习装备 / 装备切换 三个子页（两段式：先选角色再成长）
8. EXP/升级/点数写回 player_roster.json；装备槽位与授予技能
9. 部署阶段读取全局编成

### P3 系统组件
10. 字体管理（中文显示）；~~i18n 中英切换~~（已取消，仅中文版）
11. ScrollableList 通用滚动组件（战斗日志/成长界面）
12. 战斗日志颜色区分、滚动；SaveManager

### P4 内容与工具
13. summon/revive effect 实现（数据已配置，EffectSystem 只实现了 damage/heal/buff）
14. Aura/Counter 触发完善、A* 路径、AOE/位移技能
15. 像素小人资源与批量生成工具迁移
16. 关卡/剧情内容扩展
...
1. **M1 可玩闭环（Godot）**：单战场可玩（选中→移动→攻击→技能→敌方回合→胜负）→ 验证通过后
2. **M2 双战场**：DualGrid + 跨战场攻击规则 + 攻击范围高亮适配
3. **M3 流程补齐**：主菜单 → 选关 → 部署 → 战斗 → 结算
4. **M4 成长/装备**：角色选择 + 属性/技能/装备三子页 + 写回 JSON
5. **M5 系统完善**：滚动列表、战斗日志增强、存档（不做 i18n，仅中文版）
6. **M6 内容扩展**：summon/revive、Aura/Counter 完善、A*、AOE、多关卡、像素资源
