# 通用技能：疾风电涌 — 每次普攻击中叠1层（至多5），满层后下一次普攻追加50%攻击力真实特效并清层。
extends CodeSkill


func _init() -> void:
	name = "疾风电涌"
	desc = "每次普攻击中叠 1 层（至多 5 层）；满层后的下一次普攻追加 50% 攻击力的真实特效伤害并清层。"
	trigger = "on_attack"
...
func execute(user, targets, game, battle) -> Array:
	...
	if bool(rt.notes.get("frenzy_charged", false)):
		...
		return [{"target": t, "report": SkillKit.deal_effect(user, t, game, battle, {"atk": 0.5, "true": true})}]
