# 通用技能：攻坚重锤 — 对敌方最大生命最高的一员进行普攻时，追加自身最大生命 30% 的技能伤害。
extends CodeSkill


func _init() -> void:
	name = "攻坚重锤"
	desc = "对敌方最大生命最高的一名单位进行普攻时，追加自身最大生命 30% 的技能伤害。"
	trigger = "on_attack"
...
func execute(user: Unit, targets: Array, game = null, battle = null) -> Array:
	...
	if t == SkillKit.max_hp_enemy(battle, user):
		return [{"target": t, "report": SkillKit.deal_effect(user, t, game, battle, {"maxhp": 0.30, "damage_kind": DamageSystem.SKILL})}]
	return []
