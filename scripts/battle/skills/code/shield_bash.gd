# 通用技能：盾辉反击 — 获得护盾后，下一次普攻追加本次实际获得护盾值 15% 的特效伤害。
...
func _init() -> void:
	name = "盾辉反击"
	desc = "获得护盾后，下一次普攻追加本次实际获得护盾值 15% 的特效伤害。"
	trigger = "on_attack"
...
func execute(user, targets, game, battle) -> Array:
	...
	return [{"target": t, "report": SkillKit.deal_effect(user, t, game, battle, {"flat": roundi(float(credit) * SHIELD_RATIO)})}]