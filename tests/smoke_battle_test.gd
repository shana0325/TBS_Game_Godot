...
func run() -> bool:
	...
	var scenario := {
		"width": 10, "height": 8,
		"player_units": [{"type": "Warrior", "pos": [1, 1], "name": "P1"}],
		"enemy_units": [{"type": "Warrior", "pos": [6, 4], "name": "E1"}],
	}
	var battle := BattleManager.new("smoke", self, [], scenario)
	battle.setup()
	...
	for i in FRAMES:
		...
		for e in events:
			...
			var from: Vector2i = e.get("from", Vector2i.ZERO)
			var to: Vector2i = e.get("to", Vector2i.ZERO)
